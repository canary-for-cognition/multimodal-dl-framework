...
class ETSeqPreprocessor(Preprocessor):
    ...
    def __fix_invalid_rows(self, data: np.ndarray) -> np.ndarray:
        """
    ...
    def __adjust(self, item: np.ndarray) -> pd.DataFrame:
        """
    ...
    def __preprocess(self, path_to_raw: str, path_to_preprocessed: str, path_to_preprocessed_augmented: str):
        ...
        for file_name in tqdm(file_names, desc="Preprocessing files at {}".format(path_to_raw)):
            ...
            if self.__collapse_fixations:
                seq = seq.groupby(self.__features["fixations"]).size()
                seq = seq.reset_index().rename(columns={0: 'FixationLength'})
            else:
                # Drop useless features
                seq = seq.drop(labels=self.__features["event"] + self.__features["validity"], axis=1)

            # Save base sequences
            seq.to_pickle(os.path.join(path_to_preprocessed, item_id + ".pkl"))

            # Save augmented sequences
            if self.__use_cyclic_split:
                self.__cyclic_split(seq, path_to_preprocessed_augmented, item_id)
